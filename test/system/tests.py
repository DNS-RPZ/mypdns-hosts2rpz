...
#
# This file is part of hosts2rpz.
#
# hosts2rpz is free software: you can redistribute it and/or modify
# it under the terms of the GNU General Public License as published by
# the Free Software Foundation, either version 3 of the License, or
# (at your option) any later version.
#
# hosts2rpz is distributed in the hope that it will be useful,
# but WITHOUT ANY WARRANTY; without even the implied warranty of
# MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
# GNU General Public License for more details.
#
# You should have received a copy of the GNU General Public License
# along with hosts2rpz.  If not, see <https://www.gnu.org/licenses/>.
...
@unittest.skipUnless(mypdns_hosts2rpz, "mypdns_hosts2rpz not installed")
...
class FunctionalTestCase(TestCase):
    ...
    etc_mypdns_hosts2rpz_ini = Path("/etc/mypdns-hosts2rpz.ini")
    ...
    def setUp(self):
        ...
        if self.etc_mypdns_hosts2rpz_ini.exists():
            self.etc_mypdns_hosts2rpz_ini.unlink()
...
class T1CommandLineTests(FunctionalTestCase):
    def _run_mypdns_hosts2rpz(
        self, args, noise="--silent", origin=test_origin, success=True
    ):
        proc = subprocess.run(
            [
                mypdns_hosts2rpz,
    ...
    def test_invalid_origin(self):
        self._run_mypdns_hosts2rpz(
            ["--preview"], origin="rpz.example.com", success=False
        )

    def test_preview(self):
        proc = self._run_mypdns_hosts2rpz(["--preview"])
    ...
    def test_preview_with_block_subdomains(self):
        proc = self._run_mypdns_hosts2rpz(["--preview", "--subdomains"])
        self.assert_zone_prelude(proc.stdout)
    ...
    def test_preview_with_custom_serial(self):
        proc = self._run_mypdns_hosts2rpz(["--preview", "--serial", "1337"])
        self.assert_zone_prelude(proc.stdout)
        self.assertIn("1337", proc.stdout)

    def test_output(self):
        self._run_mypdns_hosts2rpz(["-z", self.zone_path])
        with self.zone_path.open("r") as zone_file:
            self.assert_zone_prelude(zone_file.read())

    def test_output_in_raw_format(self):
        self._run_mypdns_hosts2rpz(["-z", self.zone_path, "-f", "raw"])

    def test_output_in_map_format(self):
        self._run_mypdns_hosts2rpz(["-z", self.zone_path, "-f", "map"])

    def test_silent(self):
        proc = self._run_mypdns_hosts2rpz(["-z", self.zone_path], noise="--silent")
        self.assertEqual("", proc.stdout)
        self.assertEqual("", proc.stderr)

    def test_verbose(self):
        proc = self._run_mypdns_hosts2rpz(["-z", self.zone_path], noise="--verbose")
...
class T2ConfigFileTests(FunctionalTestCase):
    def setUp(self):
        super().setUp()
        self.mypdns_hosts2rpz_ini = self.named_dir / "hosts2rpz.ini"

    def _run_mypdns_hosts2rpz(self, success=True):
        proc = subprocess.run(
            [
                mypdns_hosts2rpz,
                "--verbose",
                "--config",
                self.mypdns_hosts2rpz_ini,
            ],
            stdout=subprocess.PIPE,
            stderr=subprocess.PIPE,
            universal_newlines=True,
    ...
    def _install_config(self, format="text"):
        text = f"""
            ...
            {self.example_list_two}
        """
        write_config(text, self.mypdns_hosts2rpz_ini)
...
class T3AcceptanceTests(FunctionalTestCase):
    """
    Simulate a user setting up mypdns_hosts2rpz by first playing with the
    command line arguments, then installing a config file, and finally
    by testing name resolutions are actually blocked.
    """

    cache_dir = Path(os.getenv("CACHE_DIR", "/var/cache/mypdns-hosts2rpz"))
    ...
    def _run_mypdns_hosts2rpz(self, args=None, noise="--silent", success=True):
        proc = subprocess.run(
            [mypdns_hosts2rpz, noise] + (args or []),
            stdout=subprocess.PIPE,
            stderr=subprocess.PIPE,
            universal_newlines=True,
        )
    ...
    def _test_command_line_preview(self):
        proc = self._run_mypdns_hosts2rpz(
            [
                "-t",
                self.example_list_tld,
                "-l",
        ...
        self.assertFalse(self.etc_mypdns_hosts2rpz_ini.exists())
        self.assert_zone_prelude(proc.stdout)
        self.assertIn("foo.example.net", proc.stdout)
        self.assertIn("bar.example.net", proc.stdout)
    ...
    def _test_init_config(self):
        self._run_mypdns_hosts2rpz(["--init"])
        self.assertTrue(self.etc_mypdns_hosts2rpz_ini.exists())

    def _test_init_config_again(self):
        self._run_mypdns_hosts2rpz(["--init"], success=False)
        self.assertTrue(self.etc_mypdns_hosts2rpz_ini.exists())
    ...
    def _test_config_file_output(self):
        text = f"""
            ...
            {self.example_list_two}
        """
        write_config(text, self.etc_mypdns_hosts2rpz_ini)
        self._run_mypdns_hosts2rpz()

    def _test_reload(self):
        self._run_mypdns_hosts2rpz(["--reload"])
    ...
    def test_cron_workflow(self):
        ...
        hosts2rpz, plays with the command line flags, settles on a
        particular config file, then runs hosts2rpz as root - Possibly
        as a cron job.
        """
        proc = None
        try:
            self._test_command_line_preview()
            self._test_command_line_output(
                ["-z", self.zone_path, "-u", named_user, "-g", named_user]
            )
    ...
    def test_user_workflow(self):
        """
        A user may want to run hosts2rpz as an unprivileged user, so
