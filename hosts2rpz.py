...
logger = logging.getLogger("hosts2rpz")
...
default_pihole_adlists = [
    "https://raw.githubusercontent.com/mypdns/porn-records/master/active_domains/output/domains/ACTIVE/list",
...
[main]
cache_dir     = /var/cache/mypdns-hosts2rpz
disable_cache = off
...
# https://github.com/DNS-RPZ/mypdns-hosts2rpz/blob/version-0.x/config/rpz-loggers.ini
#
...
#
# hosts2rpz will fetch these urls on each run. If a url contains
...
class Settings:
    ...
    This class will look for the config file at /etc/mypdns-hosts2rpz.ini by
    ...
    def __init__(self):
        ...
        for dest in self.make_available_for_interpolation:
            ...
            self.cfg_parser.set(
                meta.section, dest, None if value is None else str(value)
            )
    ...
    def catalog(self):
        ...
        self.add_setting(
            ...
            dest="config_path",
            type=Path,
            default="/etc/mypdns-hosts2rpz.ini",
        ...
        self.add_setting(
            ...
            dest="cache_dir",
            type=Path,
            default="/var/cache/mypdns-hosts2rpz",
            section=self.main_section,
        )
        self.add_setting(
            "-D",
    ...
    def add_setting(self, *args, dest=None, section=None, required=False, **kwargs):
        """
        ...
        if args and section:
            self.arg_parser.add_argument(*args, dest=dest, default=None, **kwargs)
        elif args:
            self.arg_parser.add_argument(*args, dest=dest, default=default, **kwargs)
...
def pipeline_debugger(pipeline, debug_pipelines):
    ...
    if pipeline_name in debug_pipelines:
        pl_logger = logging.getLogger("hosts2rpz.pipeline." + pipeline_name)
    ...
    for list_url in list_urls:
        ...
        try:
            with urlopen(req) as res, NamedTemporaryFile("w") as temp_file:
                _handle_response(cache_path, pipeline, req_headers, res, temp_file)
        except HTTPError:
            pass
...
def download_block_lists(settings, cache_dir, disable_cache):
    ...
    """
    pipeline = compose(
        [pl_to_uppercase, pl_omit_xn_top_level_domains, pl_normalize],
        settings.debug_pipelines,
    )
    download_lists(pipeline, cache_dir, disable_cache, settings.tld_list_url)
    ...
    download_lists(pipeline, cache_dir, disable_cache, *settings.block_list_urls)
...
def collect_domains(settings, cache_dir):
    ...
    logger.info(
        "obtained %d domains from %d %s",
        len(domains),
        num_lists,
        "list" if num_lists == 1 else "lists",
    )
    ...
    pipeline = compose(
        ...
        else [pl_omit_long_tokens(max_token_length), pl_sort_by_rdn],
        settings.debug_pipelines,
    )
...
def _run_command(command_list):
    proc = subprocess.run(
        command_list, stdout=PIPE, stderr=STDOUT, universal_newlines=True
    )
...
def write_zone(settings, zone: Zone, zone_name, zone_path):
    ...
    if zone_path is not None and format == "text":
        command_list = [_get_command("named-checkzone"), zone_name]
        ...
        command_list = [
            _get_command("named-compilezone"),
            "-F",
            format,
            "-o",
            zone_path,
...
def _ensure_cache_dir(cache_dir):
    try:
        ...
        logger.exception(
            "cache_dir %s does not exist or could not be created", cache_dir
        )
        raise CommandExitFailure from ex
